	...
	<body>
		<div style=" border: 1px solid #166ead;margin: 0 auto;min-height: 100%;width: 98%; display:inline-block; background:#166ead">
	    	...
	    	<div style="background: #fff;margin: 0 auto;min-height: 200px;padding: 3% 2%;width: 88%;">
				<b>Kích hoạt đăng ký tài khoản tại website {{ucwords(CGlobal::web_name)}}.</b><br/><br/>
				Chào: <b>{{$data['customer_name']}}</b><br/><br/>
				Dưới đây là thông tin đăng nhập:<br/>
				<b>Tên đăng nhập:</b> {{$data['customer_email']}}<br/>
				<b>Mật khẩu:</b> {{$data['customer_password']}}<br/><br/>
				<a href="{{URL::route('customer.pageActiveRegister')}}?k={{$data['key_secret']}}">Bấm vào để kích hoạt tài khoản</a><br/><br/>
	    	</div>
	  		<div style="max-height: 34px; height:34px; width: 100%;">
		        <div style="margin: 0 auto;width: 100%;">
		            <i style="color:#fff; padding-right: 15px;float: right; padding-top: 10px;">&copy; <a style="text-decoration: none; color:#fff;" href="{{URL::route('site.home')}}">{{ucwords(CGlobal::web_name)}}</a> 2015-{{date('Y')}}.</i>
		        </div>
    		</div>
	  	</div>
	</body>
